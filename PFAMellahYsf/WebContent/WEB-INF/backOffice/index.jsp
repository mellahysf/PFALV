...
<div>
		...
		<c:forEach items="${ categories }" var="categorie">
			...
			<tr>
				<td><img alt="${categorie.categorie }"
					src="assets/images/${categorie.icon }"></td>
				<td>${categorie.categorie }</td>

				<td><c:choose>
						<c:when test="${categorie.archiver==true }">
							<a href="Admin?idC=${categorie.id}&archiver=0" class="archivage">Reactiver</a>
						</c:when>
						<c:when test="${categorie.archiver==false }">
							<a href="Admin?idC=${categorie.id}&archiver=1" class="archivage">Archiver</a>
						</c:when>
					</c:choose></td>
			</tr>
		</c:forEach>
	...
	<table align="center">
		...
		<c:forEach items="${ listeModule }" var="module">
			...
			<tr>
				<td><img alt="${module.module }" src="assets/images/${module.image }"></td>
				<td>${module.module }</td>
				<td><c:choose>
	...
	<div class="masquer updateMatiere">
		<div id="selectMatiereForUpdate">
			<h2>Selectionner une matiére pour la modification</h2>
			<form method="post">
